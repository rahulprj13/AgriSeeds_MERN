  ...
  const AdminProducts = () => {
    ...
    const validate = () => {
      ...
      if (form.currentPrice && form.currentPrice < 0) tempErrors.currentPrice = "Current price cannot be negative";
          ...
          <section className="bg-white rounded-2xl border border-slate-200 shadow-xl p-6 md:p-8 animate-in slide-in-from-top duration-300">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                {/* Price */}
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Actual Price (₹)</label>
                ...
                {/* currentPrice*/}
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Current Price/Selling Price (₹)</label>
                  <input type="number" name="currentPrice" value={form.currentPrice} onChange={handleChange} className={`w-full bg-slate-50 border ${errors.currentPrice? 'border-red-500' : 'border-slate-200'} rounded-xl px-4 py-3 text-sm outline-none font-semibold`} />
                  {errors.currentPrice && <p className="text-[10px] text-red-500 font-bold flex items-center gap-1 mt-1"><AlertCircle size={12} /> {errors.currentPrice}</p>}
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <select name="status" value={form.status} onChange={handleChange} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-semibold">
              ...
              <div className="flex justify-end gap-3 border-t pt-6">
                <button type="button" onClick={resetForm} className="px-6 py-3 text-sm font-bold text-slate-400 hover:text-slate-600">Cancel</button>
                <button type="submit" disabled={loading} className="bg-slate-900 text-white px-10 py-3 rounded-xl font-black text-sm shadow-xl hover:bg-slate-800 transition-all">
        ...
        <div className="space-y-4">
          <div className="flex flex-col md:flex-row gap-4">
            <div className="relative flex-1 group">
              ...
              <input type="text" placeholder="Search by name..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full bg-white border border-slate-200 rounded-2xl pl-12 pr-4 py-4 text-sm font-bold shadow-sm outline-none focus:ring-2 focus:ring-green-500/10" />
            </div>
            <div className="flex items-center gap-2 bg-white border border-slate-200 rounded-2xl px-4 py-2 shadow-sm shrink-0">
              <Filter size={18} className="text-slate-400" />
              <select value={filterCategory} onChange={(e) => setFilterCategory(e.target.value)} className="bg-transparent text-sm font-black text-slate-600 outline-none">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden overflow-x-auto">
            <table className="w-full text-left">
              <thead className="bg-slate-50/50 border-b border-slate-100">
                <tr>
                  ...
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Actual Price</th>
                  <th className="px-6 py-4 text-[10px] font-black text-slate-400 uppercase tracking-widest text-center">Current Price</th>
                ...
                {filteredProducts.map((p) => {
                  ...
                  return (
                    <tr key={p._id} className="hover:bg-slate-50/80 transition-all group">
                      ...
                      <td className="px-6 py-4 text-center font-black text-slate-800">₹{p.currentPrice}</td>
            ...
            {filteredProducts.length === 0 && (
              <div className="p-20 text-center text-slate-300 font-bold uppercase tracking-widest text-xs">No products found</div>
            )}
          </div>
        </div>
      </div>
